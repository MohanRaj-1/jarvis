...
func TestReleaseNotesPrompt(t *testing.T) {
	...
	for _, want := range []string{"Return valid JSON only.", `"summary"`, `"features"`, `"fixes"`, `"changes"`, `"breaking_changes"`, "under 300 characters", "under 200 characters", "v0.5.2", "HEAD", "abcdef0", "Mohan Raj", "2026-08-06T00:00:00Z", "feat(git): add release notes", "internal/ai/git/release_notes.go", "Added"} {
		if !strings.Contains(prompt, want) {
			t.Errorf("prompt does not contain %q", want)
		}
	}
